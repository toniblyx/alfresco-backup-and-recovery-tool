...
function restoreMysqlAtPointInTime (){
		echo "$LOG_DATE_LOG - $BART_LOG_TAG - Command: $DUPLICITYBIN restore --restore-time $RESTOREDATE --file-to-restore $DBNAME.dump $DEST/$DBTYPE /tmp/$DBNAME.dump.gz" >> $ALFBRT_LOG_FILE
...
function restoreSelectedNode (){
		echo "$LOG_DATE_LOG - $BART_LOG_TAG - Command: $DUPLICITYBIN restore --restore-time $RESTOREDATE --file-to-restore $NODE_URL $DEST/$DBTYPE /tmp/$NODE_FILE_NAME" >> $ALFBRT_LOG_FILE
